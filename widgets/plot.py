...
class Line_plot(threading.Thread):
    ...
    def p1_regin_update(self):
        ...
        self.on_click_regin = False
    def update_single_data(self, curve_name, new_data_point):
        self.data_dict[curve_name].append(new_data_point)
        self.new_data = True
    # 更新p2的range的槽函数
        ...
        if self.new_data:
            ...
            if not self.on_click_regin:
                if self.cur_x > self.attention_range:
                    ...
                    #self.region.setRegion([self.cur_x,self.cur_x-self.attention_range])
                else:
                    self.region.setRegion([self.cur_x-19,self.cur_x+1])
            self.new_data = False
        else:
